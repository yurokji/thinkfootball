...
- [ ] Add basic CI or local scripts to build and run (optional if time-constrained).
...
- [ ] Implement deterministic tick loop (fixed `dt`), seeded RNG service for outcome sampling/replay.
- [ ] Implement pitch zoning (5 lanes × 4 bands + special zones) and helpers to query zones for positions.
- [ ] Add JSON save/load for `WorldState` + RNG seed to enable mid-match saves/replays.

### M2 – Movement & Ball Minimal
- [ ] Implement `MovementBase` interface and `MovementArcade` stage-0 (seek/arrive, clamp speed, facing lerp).
- [ ] Add simple player separation/avoidance to prevent overlap.
- [ ] Implement ball state machine (`CONTROLLED`, `FREE_GROUND`, `FREE_AIR`) with light damping; keep Y as render parameter only.

### M3 – AI Architecture
- [ ] Implement `PlayerBrain` / `TeamBrain` / `GroupContext` scaffolding; decouple decision (brain) from execution (movement).
- [ ] Player tick pipeline: `Brain -> Intent -> Movement -> State` respecting stats/condition constraints.
- [ ] Basic team tactics parameters (line height, press intensity, width, directness, tempo) stored in `TeamState`.
...
- [ ] Lob/cross model: similar error model using `passLong`; generate landing point and hang time profile.
- [ ] Trap/first-touch resolution: success vs heavy-touch vs lose, based on firstTouch/composure/pressure/fatigue/bounce.
- [ ] Heading resolution: contact/on-target/off/whiff using cross quality + jump/targeting/heading + pressure.
- [ ] Expose event logs for debugging/replay.

### M5 – Rendering & UI (raylib + ImGui)
...
- [ ] Packaging instructions: build/run commands in README; note third-party licenses (imgui, raylib).
